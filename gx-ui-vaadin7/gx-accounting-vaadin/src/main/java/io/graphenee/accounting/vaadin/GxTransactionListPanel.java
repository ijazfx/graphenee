...
public class GxTransactionListPanel extends AbstractEntityListPanel<GxTransactionBean> {
	...
	protected List<GxTransactionBean> fetchEntities() {
		...
		if (namespaceBean != null)
			return accountingDataService.findAllTransactionsByNamespaceAndDateRangeOrderByDateAsc(namespaceBean, fromDate, toDate);
		return accountingDataService.findAllTransactionsByDateRangeOrderByDateAsc(fromDate, toDate);
	}
	...
	protected <F> List<GxTransactionBean> fetchEntities(F filter) {
		...
		if (filter instanceof GxNamespaceBean) {
			return accountingDataService.findAllTransactionsByNamespaceAndDateRangeOrderByDateAsc((GxNamespaceBean) filter, fromDate, toDate);
		}
