...
public class BeanFaultToBeanConverter<ID, T> implements Converter<T, BeanFault<ID, T>> {
	...
	public BeanFaultToBeanConverter() {
		this("oid");
	}

	public BeanFaultToBeanConverter(String idProperty) {
		this.idProperty = idProperty;
	}

	@SuppressWarnings("unchecked")
	@Override
	public Result<BeanFault<ID, T>> convertToModel(T value, ValueContext context) {
		if (value == null)
			return Result.ok(null);
		ID id = (ID) new KeyValueWrapper(value).valueForKeyPath(idProperty);
		BeanFault<ID, T> beanFault = BeanFault.beanFault(id, value);
		return Result.ok(beanFault);
	}

	@Override
	public T convertToPresentation(BeanFault<ID, T> value, ValueContext context) {
		if (value == null)
			return null;
