 ...
 *
 */
public class Java8LocaleNegotiationStrategy implements LocaleNegotiationStrategey {
	...
	public Locale negotiate(List<Locale> supportedLocales, VaadinRequest vaadinRequest) {
		...
		try {
			...
			Class<?> languageRange = Class.forName("java.util.Locale$LanguageRange");
			Method parse = languageRange.getMethod("parse", String.class);
			Object priorityList = parse.invoke(null, languages);
			Method lookup = Locale.class.getMethod("lookup", List.class, Collection.class);
