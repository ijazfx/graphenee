...
public class Java7LocaleNegotiationStrategy implements LocaleNegotiationStrategey {

	@Override
	public Locale negotiate(final List<Locale> supportedLocales, VaadinRequest vaadinRequest) {
		String languages = vaadinRequest.getHeader("Accept-Language");
		ArrayList<Locale> preferredArray = new ArrayList<>(supportedLocales);
		if (languages != null) {
			...
			Collections.sort(preferredArray, new Comparator<Locale>() {
				...
				public int compare(Locale o1, Locale o2) {
					int pos1 = supportedLocales.size(), pos2 = supportedLocales.size();
