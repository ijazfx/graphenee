...
public class VaadinLocale {
	...
	public void setVaadinRequest(VaadinRequest vaadinRequest) {
		...
		bestLocaleByAcceptHeader = localeNegotiationStrategey.negotiate(supportedLocales, vaadinRequest);
		updateVaadinLocale();
	}
	...
	private void recursiveSetLocale() {
		...
		while (!stack.isEmpty()) {
			...
			if (component instanceof HasComponents) {
				for (Iterator<Component> i = ((HasComponents) component).iterator(); i.hasNext();) {
					stack.add(i.next());
				}
			}
