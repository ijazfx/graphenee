...
public class LazyComboBox<T> extends TypedSelect<T> {
	 ...
	 */
	public LazyComboBox(Class<T> elementType, final FilterablePagingProvider<T> filterablePageProvider, final FilterableCountProvider countProvider) {
		this(elementType, filterablePageProvider, countProvider, LazyList.DEFAULT_PAGE_SIZE);
	}
	 ...
	 */
	public LazyComboBox(Class<T> elementType, final FilterablePagingProvider<T> filterablePageProvider, final FilterableCountProvider countProvider, int pageLength) {
		this();
	...
	protected final ComboBox initList(Class<T> elementType, FilterablePagingProvider<T> filterablePageProvider, FilterableCountProvider countProvider1, int pageLength) {
		...
		piggybackLazyList = new LazyList<>(new LazyList.PagingProvider<T>() {
			...
			public List<T> findEntities(int firstRow) {
				return fpp.findEntities(firstRow, getCurrentFilter());
			}
	...
	public void loadFrom(FilterablePagingProvider<T> filterablePagingProvider, FilterableCountProvider filterableCountProvider, int pageLength) {
		...
		setBic(new DummyFilterableListContainer<T>(getType(), piggybackLazyList));
	 ...
	 */
	private static class DummyFilterableListContainer<T> extends ListContainer<T> implements Filterable {
