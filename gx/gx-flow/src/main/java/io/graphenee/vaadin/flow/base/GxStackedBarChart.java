...
public class GxStackedBarChart extends VerticalLayout {
	...
	public GxStackedBarChart() {
		...
		chart = ApexChartsBuilder.get().withColors(colors).withFill(FillBuilder.get().withColors(Arrays.asList(colors)).build()).withChart(ChartBuilder.get().withType(Type.bar).withStacked(true).withStackType(StackType.normal).build()).withPlotOptions(PlotOptionsBuilder.get().withBar(BarBuilder.get().withHorizontal(false).withColumnWidth("80%").build()).build())
				.withDataLabels(DataLabelsBuilder.get().withEnabled(false).build()).withLegend(LegendBuilder.get().withShow(false).withPosition(Position.top).withHorizontalAlign(HorizontalAlign.left).build()).build();
		add(chart);
